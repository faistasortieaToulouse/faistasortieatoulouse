...
export default function ContactPage() {
  ...
  // --- Attente que Turnstile soit prêt ---
  useEffect(() => {
    if (window.turnstile) {
      window.turnstile.ready(() => {
        console.log('Turnstile prêt !');
  ...
  const onSubmit = useCallback(
    async (data: ContactFormValues) => {
      ...
      if (!turnstileReady) {
        toast({
          variant: 'destructive',
          title: 'Turnstile non prêt',
          description: 'Veuillez patienter avant d’envoyer le formulaire.',
        });
        return;
      }
  ...
  if (!siteKey) {
    console.error('Clé publique Turnstile manquante !');
  }
        ...
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              {/* Bouton d’envoi */}
              <Button type="submit" className="w-full" disabled={form.formState.isSubmitting || !turnstileReady}>
