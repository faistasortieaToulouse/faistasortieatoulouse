...
import { Users, Calendar as CalendarIcon, BellRing } from "lucide-react";
    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <section className="flex flex-wrap justify-center gap-4 p-4 rounded-lg bg-gray-50 dark:bg-gray-800 border">
        <div className="flex flex-wrap justify-center gap-4 w-full">
          <Link href="https://play.google.com/store/apps/details?id=com.votre.appli.android" target="_blank" className="flex items-center space-x-2 p-3 bg-green-600 text-white rounded-lg shadow-md hover:bg-green-700 transition duration-300">
            <Store className="h-5 w-5" />
            <Image src="/images/google-play-badge.png" alt="Disponible sur Google Play" width={180} height={53} />
          </Link>

          <Link href="/votre-application.apk" download className="flex items-center space-x-2 p-3 bg-blue-600 text-white rounded-lg shadow-md hover:bg-blue-700 transition duration-300">
            <Store className="h-5 w-5" />
            <span className="leading-tight">
              Télécharger le fichier APK (TWA)<br/>
              <span className="text-sm font-normal opacity-90">pour Android</span>
            </span>
          </Link>

          <Link href="/install-pwa-ios" className="flex items-center space-x-2 p-3 bg-white text-gray-800 border border-gray-300 rounded-lg shadow-md hover:bg-gray-100 transition duration-300 dark:bg-gray-700 dark:text-white dark:border-gray-600">
            <Apple className="h-5 w-5" />
            <span className="font-semibold">Installer l'Appli sur iPhone (PWA)</span>
          </Link>

          <Button onClick={handleShare} className="flex items-center space-x-2 p-3 bg-primary text-primary-foreground rounded-lg shadow-md hover:bg-primary/90 transition duration-300">
