...
          >
            {/* ✅ Popup visible au clic - MODIFIÉ */}
            <Popup>
              <strong className="block mb-1">{ev.name}</strong>
              <span className="block text-sm text-gray-600">{ev.location}</span>
              
              {/* Affichage conditionnel de la date et de l'heure */}
              {(ev.date || ev.time) && (
                <div className="mt-2 pt-2 border-t border-gray-200 text-sm">
                  {ev.date && (
                    <div className="flex items-center">
                      📅 <span className="ml-1 font-semibold">{ev.date}</span>
                    </div>
                  )}
                  {ev.time && (
                    <div className="flex items-center mt-0.5">
                      🕒 <span className="ml-1">{ev.time}</span>
                    </div>
                  )}
                </div>
              )}
