...
export default function AppSidebar() {
  ...
  return (
    <div>
      {/* Mobile Header */}
      <div className="md:hidden flex items-center justify-between p-4 bg-[#F7DEEF] shadow-md">
        <Link href="/" className="flex items-center gap-2">
          <Image src={ftsLogo} alt="FTS Logo" width={36} height={36} className="rounded-full object-cover" />
          <span className="font-semibold">Fais ta Sortie</span>
        </Link>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full w-64 bg-[#F7DEEF] flex flex-col p-4 shadow-2xl transform transition-transform duration-300 z-50
        ${isOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 md:static md:flex`}
      >
        {/* Desktop Logo + GoogleTranslate */}
        <div className="flex items-center justify-between mb-6 hidden md:flex">
          <Link href="/" className="flex items-center gap-3">
            <Image src={ftsLogo} alt="FTS Logo" width={40} height={40} className="rounded-full object-cover" />
            <div>
